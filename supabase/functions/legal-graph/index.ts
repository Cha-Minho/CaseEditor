...
party.name은 관계도 원 안에 들어갈 짧은 식별자로 쓴다. 공소외 3이 경찰관이면 name은 공소외 3으로만 두고 경찰관이라는 신분과 소속은 role에 적는다. 익명 번호가 없는 기관 관계인은 형사과 경찰관, 파출소 경찰관, 검사, 관련사건 법원처럼 구분 가능한 최소 길이로 쓴다.
...
한 사람이 목적물을 찾아 가지고 오거나 새로 점유하게 된 사실은 다른 상대방이 없어도 from과 to를 그 사람으로 같은 값으로 둔 poss 관계로 만들고 생략하지 않는다. 이후 다른 사람에게 건네면 이전 점유자에서 새 점유자로 향하는 별도 poss 관계를 만든다.
목적물의 과거 점유와 현재 점유·압수 상태가 섞이지 않도록 각 점유·압수 변동 relation의 sequence를 실제 발생 순서대로 정확히 부여한다.

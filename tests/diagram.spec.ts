...
test('review and apply an AI relationship draft', async ({ page }) => {
  ...
  await expect(relationEdges).toHaveCount(0);
  ...
  await expect(relationEdges).toHaveCount(1);
  await expect(propertyEdges).toHaveCount(1);
  ...
  await expect(page.locator('.plot-party-node')).toHaveCount(2);
  ...
  await expect(page.locator('.diagram-edge:not(.property-arc)')).toHaveCount(0);
  await page.getByLabel('실행 취소', { exact: true }).click();

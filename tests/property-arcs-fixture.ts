...
const graph: LegalGraph = {
  ...
  relations: [
    ...
    { id: 'present', from: 'police', to: 'accused', label: '압수수색영장 제시', kind: 'notice', sequence: 4, objectId: 'warrant', effect: 'own', evidence: '경찰이 영장을 제시하였다', status: 'recognized', confidence: 1 }
  ],
  events: []
};
...
document.getElementById('result')!.textContent = JSON.stringify({
  arcs,
  final: arcs.filter(arc => propertyArcIsActive(arc, 4, Infinity, true))
});
